...
| <a name="input_enable_storage"></a> [enable\_storage](#input\_enable\_storage) | Enable S3 bucket and DynamoDB table for storing outputs and metrics. Required for future cost tracking features. When disabled, only IAM role and Bedrock permissions are created. | `bool` | `false` | no |
